...
class HelpTeamMatesState(AState):
    def __init__(self, trantorian):
        ...
        self.trantorian.logger.info("===========Entering Help Team mates state===========")
        ...
        while len(self.trantorian.broadcast_queue) > 0:
            ...
            if decoded is not None:
                ...
                self.trantorian.logger.info(
                    f"[HelpTeamMates] Wer found a team mate"
                )
                break
        if self.target_direction > 0:
            self.trantorian.logger.info(f"[HelpTeamMates] going to {self.target_direction}")
