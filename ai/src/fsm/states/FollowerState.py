...
class FollowerState(AState):
    def execute(self):
        ...
        if not self.trantorian.received_broadcasts:
            self.trantorian.send_command.forward() # dans le cas on a pas de broadcast, on est obligé d'en chercher un sinon on est pas follower
        ...
        if decoded:
            ...
            if "join" in instruction:
                if direction == 0:
                    ...
                    self.trantorian.broadcast_manager.create_message(self.trantorian.broadcast_manager.id, "") # pas besoin d'instruction ici normlaement
                else:
                    self.trantorian.logger.info(f"[Follower]: To leader emplacement (direction {direction})")
