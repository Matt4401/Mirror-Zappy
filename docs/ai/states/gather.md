...
### The GatherState is dedicated to searching for and collecting the missing resources for the next elevation.  
- The system generates a list of missing stones required to reach the next level.  
- If food reserves are below the defined threshold (FOOD_OPPORTUNISTIC) and a food source is visible, the Trantorian prioritizes moving to that tile to pick it up.  
- It then looks for the missing stones within its field of vision.  
- If a required stone is visible, it moves to its tile and takes it.  If no required stone is visible, it moves forward by one tile and updates its vision.  
